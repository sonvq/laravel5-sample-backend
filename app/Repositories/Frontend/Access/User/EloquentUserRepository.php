...
class EloquentUserRepository implements UserRepositoryContract
{
    ...
    public function create(array $data, $provider = false)
    {
		...
		$user->status = 1;
		$user->password = $provider ? null : bcrypt(openssl_digest($data['password'], 'sha512'));
    ...
    public function changePassword($input)
    {
        ...
        if (Hash::check(openssl_digest($input['old_password'], 'sha512'), $user->password)) {
            $user->password = bcrypt(openssl_digest($input['password'], 'sha512'));
            return $user->save();
        }
