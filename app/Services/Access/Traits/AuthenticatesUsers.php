...
trait AuthenticatesUsers
{
    ...
    public function login(LoginRequest $request)
    {
        ...
        $credentials['password'] = openssl_digest($credentials['password'], 'sha512');
    ...
    protected function handleUserWasAuthenticated(Request $request, $throttles)
    {
        ...
        $isAdmin = false;
        if (access()->user()) {
            if (count(access()->user()->roles) > 0) {
                foreach (access()->user()->roles as $singleRole) {
                    if ($singleRole->id == 1) {
                        $isAdmin = true;
                    }
                }
        ...
        if (!$isAdmin) {
            access()->logout();
            throw new GeneralException(trans('exceptions.frontend.auth.limit.normal_user'));
        }
